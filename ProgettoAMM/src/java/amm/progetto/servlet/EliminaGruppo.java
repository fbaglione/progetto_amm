    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session != null && session.getAttribute("loggedIn") != null && session.getAttribute("loggedIn").equals(true)) {
            ...
            if (request.getParameter("group") != null) {
                ...
                Group group = GroupFactory.getInstance().getGroupById(Integer.parseInt(request.getParameter("group")));
                
                // Controllo privilegi
                // accesso consentito ad admin del gruppo e amministratore
                if(group.getAdmin().equals((User) request.getAttribute("loggedUser")) || (boolean)request.getAttribute("adminPowers")) {
                    
                    // Eliminazione gruppo
                    GroupFactory.getInstance().deleteGroup(group);
                }
                else {
                    
                    // Tentativo di accesso senza privilegi
